...
if ( ! class_exists('pkg_articlesanywhereInstallerScript'))
{
    class pkg_articlesanywhereInstallerScript
    {
        ...
        private static function getMainXmlFile()
        {
            ...
            switch ($extension->type)
            {
                ...
                case 'component':
                    $xml_file = JPATH_ADMINISTRATOR . '/components/com_' . $extension->id . '/' . $extension->id . '.xml';
                    break;

                case 'module':
                    $xml_file = JPATH_SITE . '/modules/mod_' . $extension->id . '/mod_' . $extension->id . '.xml';
                    break;
        ...
        private static function hasFilesForJoomlaVersion($j_version)
        {
            $manifest = static::$adapter->getManifest();

            return count($manifest->{'files_j' . $j_version}->children());
        }
        ...
        {
            $j_version = self::getJoomlaVersion();

            if ( ! self::hasFilesForJoomlaVersion($j_version))
            {
                JFactory::getApplication()->enqueueMessage(
                    JText::sprintf(
            ...
            $min_joomla_version = static::$min_joomla_version[$j_version];
        ...
        {
            $j_version       = self::getJoomlaVersion();
            $min_php_version = static::$min_php_version[$j_version];
        ...
        private static function publishExtension($type, $name, $plugin_folder = 'system', $module_position = 'status', $client_id = 0, $force = false)
        {
            ...
            {
                case 'plugin' :
                    self::publishPlugin($name, $plugin_folder, $force);
                    break;
        ...
        private static function publishExtensions($install_type)
        {
            ...
            foreach ($extensions as $extension)
            {
                ...
                $force           = $is_admin_module || $extension->id == 'regularlabs';
